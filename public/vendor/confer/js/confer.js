;( function( window ) {
	...
	Confer.prototype.generateConversationInBar = function (info)
	{
		...
		var user_avatar = 'images/avatars/' + info.user.id + '/' + info.user.image_url;
		...
		var $li = $('<li></li>').attr('data-conversationId', info.conversation.id).attr('data-isPrivate', info.conversation.is_private), // conversationId should always be set to info.conversation.id ?
			...
			$avatar = $('<img>').addClass('confer-open-conversation-avatar').attr('src', is_group_convo ? self.options.avatar_dir + 'avatar-group.png' : user_avatar),
			$name = $('<span></span>').addClass('confer-open-conversation-name').text(is_group_convo ? makeInitialsFrom(info.conversation.name) : makeInitialsFrom(info.user.username)),
	...
	Confer.prototype.constructConversationMessage = function (message, use_timestamp)
	{
		...
		var user_avatar = 'images/avatars/' + message.sender.id + '/' + message.sender.image_url;
		...
		if (message['type'] === 'user_message')
		{
			var $message = $('<li></li>').addClass(user_is_sender ? 'confer-sent-message' : 'confer-received-message').attr('data-messageId', message.id),
				$avatar = $('<img>').addClass('confer-user-avatar').addClass(user_is_sender ? 'confer-sent-avatar' : 'confer-received-avatar').attr('src', user_avatar),
